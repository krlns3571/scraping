...
def get_result():
    ...
    try:
        lists = []
        pages = range(1,740)
        jibangs = ["lawlandkabl","eastlawkabl",'judikabl','bbjaakabl','sbjaakabl','ujblawkabl','icajakabl','ggajakabl','gwblskabl','djajakabl',
                   'cjaakabl','dgajakabl','bsajakabl','usajakabl','knjaakabl','lawgroupkabl','jjajakabl','jejujaakabl']
        for jibang in jibangs:
            for page in pages:
                driver.get(
                    f'https://kjaar.kabl.kr/modules/kabl/lawm/lawmsearch_kjaa.kabl?search=yes&sch_name=&sch_jibang={jibang}&sch_addr=&page={page}')
                # if page%10==0:
