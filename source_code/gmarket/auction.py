...
while True:
    ...
    print(page)
    driver.get(
        # f"http://minishop.gmarket.co.kr/size00/List?Title=Best%20Item&CategoryType=General&SortType=MostPopular&DisplayType=SmallImage&Page={page}&PageSize=60&IsFreeShipping=False&HasDiscount=False&HasStamp=False&HasMileage=False&IsInternationalShipping=False&IsTpl=False&MinPrice=14940&MaxPrice=1249690&Roles=System.String%5B%5D"
        f"https://stores.auction.co.kr/warmstore1/List?Title=Best%20Item&CategoryType=General&SortType=MostPopular&DisplayType=SmallImage&Page={page}&PageIndex=1&PageSize=60&IsFreeShipping=False&Is3PL=False&MinPrice=0&MaxPrice=0"
    )
    # https://stores.auction.co.kr/warmstore1/List?title=Best%20Item&sortType=MostPopular
    f"https://stores.auction.co.kr/warmstore1/List?Title=Best%20Item&CategoryType=General&SortType=MostPopular&DisplayType=SmallImage&Page={page}&PageIndex=1&PageSize=60&IsFreeShipping=False&Is3PL=False&MinPrice=0&MaxPrice=0"
    ...
    if len(driver.find_elements(By.XPATH, '//*[@id="ItemList"]/div[3]/ul/li/p/a/img')) == 0:
        break
    imgresult = ['http:' + x.get_attribute('data-original')  for x in
...
for idx, img in enumerate(imgs):
    # time.sleep(.2) /Users/nuvent/Downloads/local_projects/test_scrap/source_code
    while True:
        try:
            urlretrieve(img, f'/Users/nuvent/Downloads/local_projects/test_scrap/source_code/gmarket/pictures/{idx + 1}.jpg')
            img_ox.append('')
            break
